...
from .errors import YamlReferenceResolutionError
...
class YamlGraphParser:
    ...
    def parse(self, payload: Any, graph_id: str) -> Graph:
        ...
        if isinstance(payload, dict):
            ...
            root_node_id = self._create_generated_node("yaml-root")
            root_node = self._require_graph().nodes[root_node_id]
            root_node.set_attribute("kind", "root-sequence")
            self._visit_list(payload, root_node_id, "items")
        else:
            root_node_id = self._create_generated_node("yaml-root")
            root_node = self._require_graph().nodes[root_node_id]
            root_node.set_attribute("kind", "root-scalar")
    ...
    def _visit_list(self, values: list[Any], parent_node_id: str, relation_name: str) -> None:
        for index, value in enumerate(values):
            ...
            if isinstance(value, list):
                list_node_id = self._create_generated_node("yaml-list")
            ...
            value_node_id = self._create_generated_node("yaml-value")
    ...
    def _create_or_get_object_node(self, obj: dict[str, Any]) -> str:
        ...
        if external_id_raw is not None:
            ...
            if node_id in self._require_graph().nodes:
                raise GraphValidationError(
                    f"Duplicate node id '{node_id}' detected while parsing YAML object."
                )
        ...
        return self._create_generated_node("yaml-object")
    ...
    def _add_edge(
        ...
        edge = Edge(
            edge_id=f"yaml-edge-{self._edge_counter}",
            source_id=source_id,
            target_id=target_id,
            directed=True,
        )
    ...
    def _resolve_pending_references(self) -> None:
        ...
        if unresolved:
            ...
            raise YamlReferenceResolutionError(
                f"Unable to resolve YAML references for ids: {', '.join(unresolved_ids)}"
            )
